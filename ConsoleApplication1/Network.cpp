...
void Network::init_batch_buffers(int mini_batch_size) {
    ...
    if (!batch_buffers_allocated_) {
        ...
        for (size_t i = 0; i < layers.size(); ++i) {
            ...
            contextGPU_->allocate_batch_vector(&d_batch_deltas[i], layer_size, allocated_batch_size_);
            ...
            contextGPU_->set_to_zero_batch(d_batch_deltas[i], layer_size, allocated_batch_size_);
        }

        // Allocate targets and temp deriv buffer
        int output_size = sizes.back();
        contextGPU_->allocate_batch_vector(&d_batch_targets, output_size, allocated_batch_size_);
        contextGPU_->allocate_batch_vector(&d_temp_deriv_batch, output_size, allocated_batch_size_);
        contextGPU_->set_to_zero_batch(d_batch_targets, output_size, allocated_batch_size_);
        contextGPU_->set_to_zero_batch(d_temp_deriv_batch, output_size, allocated_batch_size_);
...
void Network::backprop_gpu_batch(const std::vector<Eigen::VectorXd>& batch_targets, double& batch_loss) {
    ...
    if (neuron_type_ == NeuronType::SIGMOID) {
        ...
        case LossType::MSE: {
            apply_deriv = true;
            contextGPU_->cost_prime_mse_crossent_batched(d_output_batch, d_batch_targets, d_batch_deltas.back(), output_size, batch_size);
            break;
        };
        case LossType::CROSS_ENTROPY: {
            apply_deriv = false;
    ...
    if (apply_deriv) {
        contextGPU_->computeActivationDerivativeGPU_batch(d_batch_pre_activations.back(), d_temp_deriv_batch, output_size, batch_size, activation_.get());
        contextGPU_->launch_elementwise_multiply_batch(d_batch_deltas.back(), d_temp_deriv_batch, d_batch_deltas.back(), output_size, batch_size);
    }
    ...
    for (int l = static_cast<int>(layers.size()) - 2; l >= 0; --l) {
        ...
        contextGPU_->computeActivationDerivativeGPU_batch(d_batch_pre_activations[l], d_temp_deriv_batch, sizes[l + 1], batch_size, activation_.get());
        contextGPU_->launch_elementwise_multiply_batch(d_batch_deltas[l], d_temp_deriv_batch, d_batch_deltas[l], sizes[l + 1], batch_size);
    }

    // Compute gradients (forward order)
    const double* d_prev_a = d_batch_main_input;
    for (size_t l = 0; l < layers.size(); ++l) {
        contextGPU_->computeGradientsGPU_batch(d_batch_deltas[l], d_prev_a, accumulate_weight_grads[l], accumulate_bias_grads[l], sizes[l + 1], sizes[l], batch_size);
...
double Network::update_mini_batch_batch(
    ...
    double batch_loss;
    backprop_gpu_batch(batch_targets, batch_loss);
    ...
    for (size_t i = 0; i < layers.size(); ++i) {
        ...
        // Update parameters
        layers[i]->update_parameters_gpu(accumulate_weight_grads[i], accumulate_bias_grads[i],
